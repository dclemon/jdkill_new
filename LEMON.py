...
class Jd_account(object):
    ...
    def login(cokstr):
        ...
        a = items.find_all('li')
        b = jd_config.getmidstring(str(a[1]), 'activity_id="', '"')
    ...
    def yuyue(sku, key, cookie):
        ...
        requests.get(url, headers=head, cookies=cookie, verify=False)
        ...
        res = requests.get(url, headers=head, cookies=cookie, verify=False)
    ...
    def gouxuan(cookie, rm, wb, sku_id, jd_req, ref, refer):
        ...
        res = jd_req.get(url=url, headers=head, cookies=cookie, verify=False)
        ...
        url = f'https://mercury.jd.com/log.gif?t=other.000000&m=UA-J2011-1&pin={pin}&uid={uid}&sid={sid}|1&v=je%3D0' \
              ...
              f'{pinid}%24jdv%3D{jdv}%24dataver%3D0.1&ref={ref}&rm={rm} '
        head = {
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) '
                          'Chrome/75.0.3770.142 Safari/537.36',
            'referer': refer
        }
        jd_req.get(url=url, headers=head, cookies=cookie, verify=False)
        return
    ...
    def getorderinfo(jd_req):
        ...
        a = jd_req.get(url=url, headers=head, verify=False)
        #sopNotPutInvoice" value="
    ...
    def submitorder(jd_req):
        ...
        dt2 = f'overseaPurchaseCookies=&submitOrderParam.payPassword={account.pwd}&vendorRemarks=[{{"venderId":"{goods.venderId}","remark":""}}]&submitOrderParam.sopNotPutInvoice={goods.needinvoice}&submitOrderParam.trackID=TestTrackId&presaleStockSign=1&submitOrderParam.ignorePriceChange=0&submitOrderParam.btSupport=0&submitOrderParam.eid={account.eid}&submitOrderParam.fp={account.fp}&submitOrderParam.jxj=1 '
        dt = f'overseaPurchaseCookies=&submitOrderParam.payPassword={account.pwd}&vendorRemarks=[{{"venderId":"{goods.venderId}","remark":""}}]&submitOrderParam.sopNotPutInvoice={goods.needinvoice}&submitOrderParam.trackID=TestTrackId&presaleStockSign=1&submitOrderParam.ignorePriceChange=0&submitOrderParam.btSupport=0&submitOrderParam.jxj=1 '
        res = jd_req.post(url=url, headers=head, data=dt2, verify=False)
        false = False
        true = True
        null = None
        print(dt2)
        ...
        if a['success']:
            ...
            time.sleep(5)
            res = jd_req.post(url=url, headers=head, data=dt, verify=False)
            a = eval(res.text)
            if a['success']:
                print('下单成功！')
                pushplus_send('抢购到订单啦！', res.text, account.pushplustoken)
...
def main():
 ...
 .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .-----------------.
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |  ________    | || |     ______   | || |   _____      | || |  _________   | || | ____    ____ | || |     ____     | || | ____  _____  | |
| | |_   ___ `.  | || |   .' ___  |  | || |  |_   _|     | || | |_   ___  |  | || ||_   \  /   _|| || |   .'    `.   | || ||_   \|_   _| | |
| |   | |   `. \ | || |  / .'   \_|  | || |    | |       | || |   | |_  \_|  | || |  |   \/   |  | || |  /  .--.  \  | || |  |   \ | |   | |
| |   | |    | | | || |  | |         | || |    | |   _   | || |   |  _|  _   | || |  | |\  /| |  | || |  | |    | |  | || |  | |\ \| |   | |
| |  _| |___.' / | || |  \ `.___.'\  | || |   _| |__/ |  | || |  _| |___/ |  | || | _| |_\/_| |_ | || |  \  `--'  /  | || | _| |_\   |_  | |
| | |________.'  | || |   `._____.'  | || |  |________|  | || | |_________|  | || ||_____||_____|| || |   `.____.'   | || ||_____|\____| | |
| |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------' 
 """)
